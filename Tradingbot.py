...
password = st.text_input("Passphrase (OKX සඳහා පමණි):", type="password")
st.markdown("---")
...
with col_a:
    symbol = st.selectbox("Trade කරන Coin එක (Symbol):", ["TRX/USDT", "BTC/USDT", "ETH/USDT", "SOL/USDT"])
with col_b:
    timeframe = st.selectbox("Timeframe එක තෝරන්න:", ["1m", "5m", "15m", "1h"])
...
if st.button("▶ START BOT"):
    if not api_key or not secret_key:
        ...
        exchange_params = {'apiKey': api_key, 'secret': secret_key, 'enableRateLimit': True}
            ...
            while True:
                try:
                    ...
                    if current_rsi < rsi_buy:
                        send_telegram_message(f"🟢 BUY Signal: {symbol}\nPrice: {current_price}\nRSI: {current_rsi:.2f}")
                        ...
                        send_telegram_message(f"🔴 SELL Signal: {symbol}\nPrice: {current_price}\nRSI: {current_rsi:.2f}")
                    ...
                    placeholder.warning(f"දත්ත ලබා ගැනීමේදී සුළු දෝෂයක්. Error: {e}")
                    
                time.sleep(15) 
                
        except Exception as e:
            st.error(f"Exchange එකට සම්බන්ධ වීමේදී දෝෂයක්. Error: {e}")
